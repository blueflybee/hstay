package com.qtec.homestay.data.mapper;
...
import com.qtec.homestay.domain.model.lock.model.core.SubBlePkg;
import com.qtec.homestay.domain.model.lock.model.rsp.BleAddLockFpResponse;
import com.qtec.homestay.domain.model.lock.model.rsp.BleAddLockPwdResponse;
import com.qtec.homestay.domain.model.lock.model.rsp.BleCheckLockAdminPwdResponse;
import com.qtec.homestay.domain.model.lock.model.rsp.BleGetLockInfoResponse;
import com.qtec.homestay.domain.model.lock.model.rsp.BleGetRemoteLockStatusResponse;
import com.qtec.homestay.domain.utils.BleHexUtil;
...
//      System.out.println("subPkgSize = " + subPkgSize);
  ...
  private byte[] encrypt(byte[] key, String toEncryptData) {
    if (ENCRYPT_SM4.equals(mEncryptionType)) {
      ...
      byte[] bytes = BleHexUtil.hexStringToBytes(toEncryptData);
      byte[] out = EncryptUtils.encryptAES(bytes, key, AES_Transformation, null);
      return out;
    }
    return null;
  }
...
//      System.out.println("encryptData.length() = " + encryptData.length());
      ...
      byte[] out = EncryptUtils.decryptAES(bytes, key, AES_Transformation, null);
      return out;
    }
    return null;
  }
  ...
  public String clearFactoryResetFlag(BlePkg pkg) {
    if (pkg == null || pkg.getBody() == null
        || TextUtils.isEmpty(pkg.getBody().getPayload())) return null;
    String payload = pkg.getBody().getPayload();
  ...
  public String unbindUser(BlePkg pkg) {
    return clearFactoryResetFlag(pkg);
  }
